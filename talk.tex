...
\begin{tikzpicture}[x=1cm,y=1cm,scale=0.95]
  ...
  \only<7>{
    ...
    \node[circle, draw=orange!80!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=orange!50] at (\bxLO,\byyLO) {};
    % Right picture, bin 2, grey ball above blue x (level 4) turns orange
    \pgfmathsetmacro{\bxRO}{1.025 + 2*(\bw+\bgap) + 0.5*\bw}
    \pgfmathsetmacro{\byyRO}{\biny + \boffset + 3*\bsp}
    \node[circle, draw=orange!80!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=orange!50] at (\bxRO,\byyRO) {};
  }
\end{tikzpicture}
}%
}
...
\begin{frame}{Full Algorithm}
  ...
  \begin{tikzpicture}[scale=1.0]
    ...
    \only<1>{
      ...
      \draw[decorate, decoration={brace, amplitude=5pt, mirror}, line width=0.7pt]
        (0, -0.1) -- (\prefixW, -0.1);
      \node[below, font=\small] at (0.5*\prefixW, -0.35) {$O(\log \log (m/n))$ rounds};
    }
