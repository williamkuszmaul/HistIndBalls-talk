...
\centering
% Left picture: Current state A_S (always visible)
...
\begin{tikzpicture}[scale=0.55]
  ...
  % Frame 3: Row of 10 balls above bins (9 grey + 1 green as 5th ball)
  \only<3>{
    \foreach \i in {0,...,9} {
  ...
  % Frame 4+: Balls in bins (greedy assignment) - 10 balls total, loads 2,3,3,2 with green in bin 3
  \only<4->{
    % Bin 0: 2 balls (grey)
    \foreach \j in {1,2} {
      \pgfmathsetmacro{\bx}{0*(\bw+\bgap)+0.5*\bw}
...
% Text area with the greedy simulation steps (appears on frame 2+)
\only<2->{%
To insert a new ball $x$, we compute $A_{S \cup \{x\}}$:
