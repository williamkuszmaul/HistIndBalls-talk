...
\newcommand{\TwoWorldFigure}[2]{% extra tikz for world0 balls, world1 balls
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,scale=0.95]
    ...
    \node[font=\normalsize\bfseries] at ({\xL+1.5*(\binw+\bingap)+0.5*\binw},-2.45) {Computing $\texttt{Greedy}(S)$};
    \node[font=\normalsize\bfseries] at ({\xR+1.5*(\binw+\bingap)+0.5*\binw},-2.45) {Computing $\texttt{Greedy}(S \cup \{x^*\})$};
...
\begin{tikzpicture}[scale=0.65]
  ...
  % === History 1: Always visible ===
  % Label changes from "History:" to "History 1:" when other histories appear
  ...
  \node[draw, circle, minimum size=1.1cm, fill=black!5, line width=1pt] (h1s3grey) at (3*\sp, \h) {};
  
  % Last circle: grey on frame 1, blue on frames 2+
  \only<1>{\node[draw, circle, minimum size=1.1cm, fill=black!5, line width=1pt] at (3*\sp, \h) {\small $\{y\}$};}
  \only<2->{\node[draw, circle, minimum size=1.1cm, fill=blue!15, draw=blue!70!black, line width=1pt] (h1s3) at (3*\sp, \h) {\small $\{y\}$};}
  ...
  % === Adversary on the right side (appears on frame 2+) ===
  \only<2->{
  ...
  % === History 2: Insert(y), Insert(x), Delete(x) - appears on frame 3+ ===
  \only<3->{
  ...
  % === History 3: ... -> {y} -> Insert(z) -> {y,z} -> Delete(z) -> {y} - appears on frame 3+ ===
  \only<3->{
...
\textbf{Yet fundamental questions remain open.}
...
\begin{tikzpicture}[scale=0.85]
  ...
  % Ball x being inserted (dark forest green, centered between its two choices)
...
\begin{frame}{A Simple History-Independent Algorithm}
...
\centering
% Left picture: bins with balls (current state) - Frames 1-6
\only<1-6>{\begin{tikzpicture}[scale=0.55]
  \def\bw{0.9}
  \def\bh{3.2}
  \def\by{0}
  \def\bgap{0.4}
  ...
  % Ball x being inserted (green, between bins 0 and 3)
  \pgfmathsetmacro{\binzeroX}{0*(\bw+\bgap)+0.5*\bw}
  \pgfmathsetmacro{\binthreeX}{3*(\bw+\bgap)+0.5*\bw}
  \pgfmathsetmacro{\xcenter}{(\binzeroX+\binthreeX)/2}
  ...
  % Arrows to two choices
  \draw[-{Stealth[length=2mm, width=1.5mm]}, green!50!black, line width=0.7pt] (xball) -- (\binzeroX,\bh);
  \draw[-{Stealth[length=2mm, width=1.5mm]}, green!50!black, line width=0.7pt] (xball) -- (\binthreeX,\bh);
  
  % Label below
  \pgfmathsetmacro{\totalw}{3*(\bw+\bgap)+\bw}
  \node[below] at ({0.5*\totalw}, -0.3) {\small Current state};
  
\end{tikzpicture}}%
% Left picture: Updated to match Computing A_S - Frame 7+
\only<7->{\begin{tikzpicture}[scale=0.55]
  \def\bw{0.9}
  \def\bh{3.2}
  \def\by{0}
  \def\bgap{0.4}
  ...
  % Invisible node to match height
  \path (0, -0.3) -- (0, 4.5);
  ...
  % Bin 0: 2 balls (1 grey + 1 green on top)
  \pgfmathsetmacro{\bx}{0*(\bw+\bgap)+0.5*\bw}
  ...
  % Label below
  \pgfmathsetmacro{\totalw}{3*(\bw+\bgap)+\bw}
  \node[below] at ({0.5*\totalw}, -0.3) {\small Updated state};
...
\begin{minipage}[b]{0.45\textwidth}
\centering
% Right picture: Computing A_S with animation
\only<4->{\begin{tikzpicture}[scale=0.55]
  \def\bw{0.9}
  \def\bh{3.2}
  \def\by{0}
  \def\bgap{0.4}
  ...
  % Invisible node to match height of left picture
  \path (0, -0.3) -- (0, 4.5);
  
  % Draw 4 rectangular bins (always visible from step 4)
  \foreach \i in {0,...,3} {
    \pgfmathsetmacro{\xx}{\i*(\bw+\bgap)}
    \draw[line width=1pt, black!80] (\xx,\by) -- (\xx,\bh);
    \draw[line width=1pt, black!80] ({\xx+\bw},\by) -- ({\xx+\bw},\bh);
  ...
  % Step 5: Row of 8 balls above bins (7 grey + 1 green in middle)
  \only<5>{
    \foreach \i in {0,...,7} {
      \pgfmathsetmacro{\ballx}{\i*0.55 + 0.3}
      \ifnum\i=4
        \node[circle, draw, minimum size=\bs, inner sep=0pt, line width=0.8pt, draw=green!50!black, fill=green!20] at (\ballx, 4.0) {};
      \else
        \node[circle, draw, minimum size=\bs, inner sep=0pt, line width=0.8pt, fill=black!15] at (\ballx, 4.0) {};
      \fi
    }
    \node[above] at (2.2, 4.4) {\scriptsize $x_1, x_2, \ldots, x_8$};
  }
  
  % Step 6: Balls in bins (greedy assignment) - 8 balls total, loads 2,2,1,3
  \only<6->{
    % Bin 0: 2 balls (1 grey + 1 green on top)
    \pgfmathsetmacro{\bx}{0*(\bw+\bgap)+0.5*\bw}
    \pgfmathsetmacro{\byy}{\by+0.35}
  ...
  % Label below
  \pgfmathsetmacro{\totalw}{3*(\bw+\bgap)+\bw}
  \node[below] at ({0.5*\totalw}, -0.3) {\small Computing $A_S$};
...
\item Observe current set $S$
\item \tikz[remember picture, baseline=(step2.base)]{\node[inner sep=0pt](step2){Compute $A_S$};}
\item Update the system to reflect $A_S$
\end{enumerate}
\end{minipage}
}%
% Step 3: Box appears
...
\begin{tikzpicture}[remember picture, overlay]
  \node[draw, rounded corners, fill=blue!5, text width=6.5cm, align=left, anchor=west] (box) at ([xshift=1.8cm, yshift=-0.2cm]step2.east) {
    \textbf{Compute $A_S$ by simulating greedy:}\\[2pt]
    \hspace{0.3cm}2a. Start with empty bins\\
    \hspace{0.3cm}2b. Sort balls in $S$ to get $x_1, x_2, \ldots$\\
    \hspace{0.3cm}2c. Insert balls using greedy
  };
  \draw[-{Stealth[length=2mm, width=1.5mm]}, blue!60!black, line width=0.8pt] (box.west) -- (step2.east);
\end{tikzpicture}
}
...
\TwoWorldFigure{ }{ }
\vfill
How does $\texttt{Greedy}(S)$ change if we add a ball $x^{\star}$?
\end{frame}
