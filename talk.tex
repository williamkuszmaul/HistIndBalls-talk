...
\begin{tikzpicture}[
  ...
  \node[portrait] (elaine) at (4, 0)
    {\includegraphics[height=1.5cm]{Elaine.jpg}};
  ...
  % Speech bubbles appearing one at a time
  \only<3->{
    \node[bubble, above=0.2cm of michael] (mb)
      {\textbf{Slice} off the excess flour\ldots};
  }
  \only<4->{
    \node[bubble, above=0.2cm of elaine] (eb)
      {\ldots and \textbf{spread} it evenly!};
  }
  \only<5->{
...
\begin{tikzpicture}[
  ...
  \only<2->{
    \node[bubble, above=0.2cm of michael] (mb)
      {\textbf{Slice} off the excess frosting\ldots};
  }
  \only<3->{
    \node[bubble, above=0.2cm of elaine] (eb)
      {\ldots and \textbf{spread} it smooth!};
  }
  \only<4->{
    \node[bubble, above=0.2cm of bill] (bb)
      {???};
...
\begin{tikzpicture}[scale=0.85]
  ...
  % ======== Overlay 2: Red tops (no slice line yet) ========
  \only<2>{
    ...
    \draw[decorate, decoration={brace, amplitude=6pt, mirror}, line width=0.8pt]
          (8.4, 2.2) -- (8.4, 3.8);
    \node[right, font=\normalsize] at (8.65, 3.0) {$\approx\Big(\dfrac{m}{n}\Big)^{1/2}$};
  }

  % ======== Overlay 3: Slice line + scissors ========
  \only<3>{
    % Draw bars in two parts: grey below slice, red above
    \foreach \i in {0,...,7} {
      \pgfmathsetmacro{\xx}{\i*(\bw+\bgap)}
    ...
    % Slice line with scissors (scissors on the left)
  ...
  \only<4>{
    ...
    % Curly brace on the right: spans 2.7 to 3.3 (tighter range)
    \draw[decorate, decoration={brace, amplitude=6pt, mirror}, line width=0.8pt]
          (8.4, 2.7) -- (8.4, 3.3);
...
\only<3-4>{
\begin{itemize}
\item Balls above dotted line $\approx (mn)^{1/2}$
\only<4>{\item By Key Fact, bin loads within $\approx (m/n)^{1/4}$}
\end{itemize}
}
...
\begin{frame}{What's the Recourse?}\pause
\noindent\hspace*{-0.5\dimexpr\paperwidth-\textwidth\relax}%
\makebox[\paperwidth][c]{%
\begin{tikzpicture}[x=1cm,y=1cm,scale=0.95]
  ...
  % Parameters (scaled down to fit two worlds side by side)
  \def\bw{0.6}
  \def\bgap{0.15}
  \def\bs{0.32cm}
  \def\bsp{0.40}
  ...
  % Blue ball x on right world, bin 2 (level 6, on top of 5 existing balls)
...
% ===== Duplicate: blue ball x inserted at level 3 in bin 2, pushes grey above line =====
...
\begin{tikzpicture}[x=1cm,y=1cm,scale=0.95]
  ...
  % Draw both worlds (bins, base grey balls, slice line, scissors, red balls)
  \foreach \wx in {-5.375, 1.025} {
    % 6 bins
    \foreach \i in {0,...,5} {
      \pgfmathsetmacro{\xx}{\wx + \i*(\bw+\bgap)}
  ...
  % === Right-world bin 2 corrections: x inserted at level 3 ===
  % Blue ball x at level 3 (covers the grey ball drawn by foreach)
  \pgfmathsetmacro{\bx}{1.025 + 2*(\bw+\bgap) + 0.5*\bw}
  \pgfmathsetmacro{\byy}{\biny + \boffset + 2*\bsp}
  \node[circle, draw=blue!80!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=blue!30, font=\tiny] at (\bx,\byy) {$x$};
  % Grey ball pushed up to level 4 (covers the red ball from foreach)
  \pgfmathsetmacro{\byy}{\biny + \boffset + 3*\bsp}
  \node[circle, draw, minimum size=\bs, inner sep=0pt, line width=0.6pt, fill=black!15] at (\bx,\byy) {};
  % Red ball pushed up to level 6 (new)
  \pgfmathsetmacro{\byy}{\biny + \boffset + 5*\bsp}
  \node[circle, draw=red!70!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=red!25] at (\bx,\byy) {};
...
% ===== Duplicate: blue ball x in bin 3, slice line moves up on right =====
\begin{frame}{What's the Recourse?}
\noindent\hspace*{-0.5\dimexpr\paperwidth-\textwidth\relax}%
\makebox[\paperwidth][c]{%
\begin{tikzpicture}[x=1cm,y=1cm,scale=0.95]
  ...
  \def\biny{-2.0}
  \pgfmathsetmacro{\wallTop}{\biny + 2.85}
  \pgfmathsetmacro{\sliceYL}{\biny + \boffset + 2.5*\bsp}
  \pgfmathsetmacro{\sliceYR}{\biny + \boffset + 3.5*\bsp}

  % Draw bin walls for both worlds
  \foreach \wx in {-5.375, 1.025} {
    \foreach \i in {0,...,5} {
      \pgfmathsetmacro{\xx}{\wx + \i*(\bw+\bgap)}
      \draw[line width=0.8pt, black!80] (\xx,\biny) -- (\xx,\wallTop);
  ...
  % === LEFT WORLD (unchanged, slice between levels 3-4) ===
  % Grey balls (3 per bin)
  \foreach \bin in {0,...,5} {
    \foreach \j in {1,...,3} {
      \pgfmathsetmacro{\bx}{-5.375 + \bin*(\bw+\bgap) + 0.5*\bw}
      \pgfmathsetmacro{\byy}{\biny + \boffset + (\j-1)*\bsp}
      \node[circle, draw, minimum size=\bs, inner sep=0pt, line width=0.6pt, fill=black!15] at (\bx,\byy) {};
    }
  }
  % Left slice line & scissors
  \draw[dashed, line width=1.2pt, red!70!black] ({-5.375 - 0.15},\sliceYL) -- ({-5.375 + 5*(\bw+\bgap) + \bw + 0.1},\sliceYL);
  \node[left, red!70!black, font=\normalsize] at ({-5.375 - 0.3},\sliceYL) {\ding{34}};
  % Left red balls above threshold (levels 4+)
  % Bin 0: level 4
  \pgfmathsetmacro{\bx}{-5.375 + 0*(\bw+\bgap) + 0.5*\bw}
  \pgfmathsetmacro{\byy}{\biny + \boffset + 3*\bsp}
  \node[circle, draw=red!70!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=red!25] at (\bx,\byy) {};
  % Bin 1: levels 4-7
  \foreach \j in {4,...,7} {
    \pgfmathsetmacro{\bx}{-5.375 + 1*(\bw+\bgap) + 0.5*\bw}
    \pgfmathsetmacro{\byy}{\biny + \boffset + (\j-1)*\bsp}
    \node[circle, draw=red!70!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=red!25] at (\bx,\byy) {};
  }
  % Bin 2: levels 4-5
  \foreach \j in {4,5} {
    \pgfmathsetmacro{\bx}{-5.375 + 2*(\bw+\bgap) + 0.5*\bw}
    \pgfmathsetmacro{\byy}{\biny + \boffset + (\j-1)*\bsp}
    \node[circle, draw=red!70!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=red!25] at (\bx,\byy) {};
  }
  % Bin 3: 0 extra
  % Bin 4: levels 4-6
  \foreach \j in {4,...,6} {
    \pgfmathsetmacro{\bx}{-5.375 + 4*(\bw+\bgap) + 0.5*\bw}
    \pgfmathsetmacro{\byy}{\biny + \boffset + (\j-1)*\bsp}
    \node[circle, draw=red!70!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=red!25] at (\bx,\byy) {};
  }
  % Bin 5: levels 4-5
  \foreach \j in {4,5} {
    \pgfmathsetmacro{\bx}{-5.375 + 5*(\bw+\bgap) + 0.5*\bw}
    \pgfmathsetmacro{\byy}{\biny + \boffset + (\j-1)*\bsp}
    \node[circle, draw=red!70!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=red!25] at (\bx,\byy) {};
  }

  % === RIGHT WORLD (slice moved up to between levels 4-5, same red balls) ===
  % Grey balls (3 per bin)
  \foreach \bin in {0,...,5} {
    \foreach \j in {1,...,3} {
      \pgfmathsetmacro{\bx}{1.025 + \bin*(\bw+\bgap) + 0.5*\bw}
      \pgfmathsetmacro{\byy}{\biny + \boffset + (\j-1)*\bsp}
      \node[circle, draw, minimum size=\bs, inner sep=0pt, line width=0.6pt, fill=black!15] at (\bx,\byy) {};
    }
  }
  % Red balls above old threshold (same as left world)
  % Bin 0: level 4
  \pgfmathsetmacro{\bx}{1.025 + 0*(\bw+\bgap) + 0.5*\bw}
  \pgfmathsetmacro{\byy}{\biny + \boffset + 3*\bsp}
  \node[circle, draw=red!70!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=red!25] at (\bx,\byy) {};
  % Bin 1: levels 4-7
  \foreach \j in {4,...,7} {
    \pgfmathsetmacro{\bx}{1.025 + 1*(\bw+\bgap) + 0.5*\bw}
    \pgfmathsetmacro{\byy}{\biny + \boffset + (\j-1)*\bsp}
    \node[circle, draw=red!70!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=red!25] at (\bx,\byy) {};
  }
  % Bin 2: levels 4-5
  ...
  % Bin 3: 0 extra (same as left)
  % Bin 4: levels 4-6
  \foreach \j in {4,...,6} {
    \pgfmathsetmacro{\bx}{1.025 + 4*(\bw+\bgap) + 0.5*\bw}
    \pgfmathsetmacro{\byy}{\biny + \boffset + (\j-1)*\bsp}
    \node[circle, draw=red!70!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=red!25] at (\bx,\byy) {};
  }
  % Bin 5: levels 4-5
  \foreach \j in {4,5} {
    \pgfmathsetmacro{\bx}{1.025 + 5*(\bw+\bgap) + 0.5*\bw}
    \pgfmathsetmacro{\byy}{\biny + \boffset + (\j-1)*\bsp}
    \node[circle, draw=red!70!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=red!25] at (\bx,\byy) {};
  }
  % Blue ball x in bin 3 at level 4
  \pgfmathsetmacro{\bx}{1.025 + 3*(\bw+\bgap) + 0.5*\bw}
  \pgfmathsetmacro{\byy}{\biny + \boffset + 3*\bsp}
  \node[circle, draw=blue!80!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=blue!30, font=\tiny] at (\bx,\byy) {$x$};
  % Right slice line & scissors (higher)
  \draw[dashed, line width=1.2pt, red!70!black] ({1.025 - 0.15},\sliceYR) -- ({1.025 + 5*(\bw+\bgap) + \bw + 0.1},\sliceYR);
  \node[left, red!70!black, font=\normalsize] at ({1.025 - 0.3},\sliceYR) {\ding{34}};

  % World labels below bins
  \node[font=\normalsize] at (-3.2, -2.55) {Computing $A_S$};
  \node[font=\normalsize] at (3.2, -2.55) {Computing $A_{S \cup \{x\}}$};

  % Overlay 2: big red X crossing out the right picture
  \only<2->{
    \draw[red!70!black, line width=3pt] (0.6, -2.3) -- (5.8, 2.6);
    \draw[red!70!black, line width=3pt] (0.6, 2.6) -- (5.8, -2.3);
  }
...
\begin{center}
\textbf{Question:} Which balls do we slice in each round?
\end{center}
...
\hspace*{-0.5cm}\parbox{\dimexpr\textwidth+0.5cm}{%
\begin{itemize}
  \item \textbf{Option 1:} Scrape off the top \hfill {\color{red}\ding{55}} \small Reuses stale randomness \normalsize
  \pause
  \item \textbf{Option 2:} Priority queue per bin \hfill {\color{red}\ding{55}} \small Exploding recourse \normalsize
  \pause
  \item \textbf{Our approach:} Option 2 + Assign every ball a \textbf{round number}
\end{itemize}
}
...
\begin{tikzpicture}[scale=0.75]
  ...
  % Extra grey balls above the slice line
  % Bin 0 (bin 1): load 4, 1 ball above line
  \pgfmathsetmacro{\bx}{0*(\bw+\bgap)+0.5*\bw}
  \pgfmathsetmacro{\byy}{\by+0.30+3*\bsp}
  \node[circle, draw, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=black!15] at (\bx,\byy) {};
  % Bin 1 (bin 2): load 7, 4 balls above line
  \pgfmathsetmacro{\bx}{1*(\bw+\bgap)+0.5*\bw}
  \foreach \j in {4,...,7} {
    \pgfmathsetmacro{\byy}{\by+0.30+(\j-1)*\bsp}
    \node[circle, draw, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=black!15] at (\bx,\byy) {};
  }
  % Bin 2 (bin 3): load 5, 2 balls above line
  \pgfmathsetmacro{\bx}{2*(\bw+\bgap)+0.5*\bw}
  \foreach \j in {4,5} {
    \pgfmathsetmacro{\byy}{\by+0.30+(\j-1)*\bsp}
    \node[circle, draw, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=black!15] at (\bx,\byy) {};
  }
  % Bin 3 (bin 4): load 3, 0 balls above line
  % Bin 4 (bin 5): load 6, 3 balls above line
  \pgfmathsetmacro{\bx}{4*(\bw+\bgap)+0.5*\bw}
  \foreach \j in {4,...,6} {
    \pgfmathsetmacro{\byy}{\by+0.30+(\j-1)*\bsp}
    \node[circle, draw, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=black!15] at (\bx,\byy) {};
  }
  % Bin 5 (bin 6): load 5, 2 balls above line
  \pgfmathsetmacro{\bx}{5*(\bw+\bgap)+0.5*\bw}
  \foreach \j in {4,5} {
    \pgfmathsetmacro{\byy}{\by+0.30+(\j-1)*\bsp}
    \node[circle, draw, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=black!15] at (\bx,\byy) {};
  ...
  % Orange round-k overlays on topmost N balls
  % Bin 0 (bin 1): top 2 → levels 3,4
  \pgfmathsetmacro{\bx}{0*(\bw+\bgap)+0.5*\bw}
  \foreach \j in {3,4} {
    \pgfmathsetmacro{\byy}{\by+0.30+(\j-1)*\bsp}
    \node[circle, draw=orange!80!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=orange!30, font=\tiny] at (\bx,\byy) {$k$};
  }
  % Bin 1 (bin 2): top 4 → levels 4,5,6,7
  \pgfmathsetmacro{\bx}{1*(\bw+\bgap)+0.5*\bw}
  \foreach \j in {4,...,7} {
    \pgfmathsetmacro{\byy}{\by+0.30+(\j-1)*\bsp}
    \node[circle, draw=orange!80!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=orange!30, font=\tiny] at (\bx,\byy) {$k$};
  }
  % Bin 2 (bin 3): 0 round-k
  % Bin 3 (bin 4): top 1 → level 3
  \pgfmathsetmacro{\bx}{3*(\bw+\bgap)+0.5*\bw}
  \pgfmathsetmacro{\byy}{\by+0.30+2*\bsp}
  \node[circle, draw=orange!80!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=orange!30, font=\tiny] at (\bx,\byy) {$k$};
  % Bin 4 (bin 5): top 3 → levels 4,5,6
  \pgfmathsetmacro{\bx}{4*(\bw+\bgap)+0.5*\bw}
  \foreach \j in {4,...,6} {
    \pgfmathsetmacro{\byy}{\by+0.30+(\j-1)*\bsp}
    \node[circle, draw=orange!80!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=orange!30, font=\tiny] at (\bx,\byy) {$k$};
  }
  % Bin 5 (bin 6): top 1 → level 5
  \pgfmathsetmacro{\bx}{5*(\bw+\bgap)+0.5*\bw}
  \pgfmathsetmacro{\byy}{\by+0.30+4*\bsp}
  \node[circle, draw=orange!80!black, minimum size=\bs, inner sep=0pt, line width=0.7pt, fill=orange!30, font=\tiny] at (\bx,\byy) {$k$};
...
\textbf{Challenge:} Some bins may not have enough round-$k$ balls above the line.
