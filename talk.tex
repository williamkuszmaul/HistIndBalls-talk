...
\only<1-6>{\begin{tikzpicture}[scale=0.55]
  ...
  % Fixed bounding box to match all frames
...
\only<7->{\begin{tikzpicture}[scale=0.55]
  ...
  % Label below
  \pgfmathsetmacro{\totalw}{3*(\bw+\bgap)+\bw}
  \node[below] at ({0.5*\totalw}, -0.3) {\small Updated state $A_{S \cup \{x\}}$};
...
\begin{minipage}[b]{0.45\textwidth}
\centering
% Right picture: Computing A_S with animation - bounding box always present
\begin{tikzpicture}[scale=0.55]
  ...
  % Fixed bounding box - always present to prevent shifting
  \pgfmathsetmacro{\totalw}{3*(\bw+\bgap)+\bw}
  \path[use as bounding box] (-0.5, -0.7) rectangle ({\totalw+0.5}, 4.5);
  
  % Draw 4 rectangular bins (visible from step 4)
  \only<4->{
    \foreach \i in {0,...,3} {
      \pgfmathsetmacro{\xx}{\i*(\bw+\bgap)}
      \draw[line width=1pt, black!80] (\xx,\by) -- (\xx,\bh);
  ...
  % Step 5: Row of 10 balls above bins (9 grey + 1 green as 5th ball)
  \only<5>{
    \foreach \i in {0,...,9} {
  ...
  % Step 6: Balls in bins (greedy assignment) - 10 balls total, loads 2,3,3,2 with green in bin 3
  \only<6->{
    % Bin 0: 2 balls (grey)
    \foreach \j in {1,2} {
      \pgfmathsetmacro{\bx}{0*(\bw+\bgap)+0.5*\bw}
  ...
  % Label below (visible from step 4)
  \only<4->{
    \pgfmathsetmacro{\totalw}{3*(\bw+\bgap)+\bw}
...
% Text area - fixed height box to prevent shifting
\hspace*{-0.5cm}
\begin{minipage}[t][2.2cm][t]{0.45\textwidth}
\only<2->{%
At each time step:
\begin{enumerate}\setlength{\itemsep}{0pt}
\item Observe current set $S$
\item \tikz[remember picture, baseline=(step2.base)]{\node[inner sep=0pt](step2){Compute $A_{S \cup \{x\}}$};}
\item Update the system to reflect $A_{S \cup \{x\}}$
\end{enumerate}
}%
\end{minipage}
% Step 3: Box appears (overlay so it doesn't affect layout)
\only<3->{%
\begin{tikzpicture}[remember picture, overlay]
  \node[draw, rounded corners, fill=blue!5, text width=6.5cm, align=left, anchor=west] (box) at ([xshift=1.8cm, yshift=-0.2cm]step2.east) {
    \textbf{Compute $A_{S \cup \{x\}}$ by simulating greedy:}\\[2pt]
